...
- [x] 1.1 Verify the scaled residual target-selection evidence exists, is leak-clean, and selects `clarify/slots`.
- [x] 1.2 Load the source cluster inspection and isolate the selected clarify slot cluster by task family and field path.
...
- [x] 2.1 Generate a public-safe JSON/Markdown candidate design report with stable candidate theme ids, source family coverage, accepted target sketches, rejected drift sketches, and suggested public-safe utterance templates.
- [x] 2.2 Cover search/extract ambiguity, navigation/form-fill ambiguity, and pronoun/context-missing ambiguity as separate candidate themes.
...
- [x] 3.1 Refresh `CONTEXT.md` with the clarify candidate-design result and strict claim boundaries.
