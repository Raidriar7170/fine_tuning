...
- [x] 3.1 Generate `reports/public-sample/a100-public-readonly-search-policy-train-split-rerun/` with predictions, metadata, prompt snapshot, raw decoded summary, generation trace, train-split gold rows, metrics, schema guard summary, public-readonly search policy diagnosis, manifest, report, and leak scans.
- [x] 3.2 Add focused tests for evidence shape, privacy boundaries, strict metrics, prompt-policy visibility, row-level field counts, and no semantic-equivalence/slot-normalization/re-score claims.
