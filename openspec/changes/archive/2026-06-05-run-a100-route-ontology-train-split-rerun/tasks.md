...
- [x] 1.1 Confirm isolated worktree status, branch, baseline tests, OpenSpec state, and explicit A100 authorization.
...
- [x] 2.3 Run `voice2task.cli.train sft-predict` in prediction-only mode using the existing private adapter and current route ontology prompt code.
...
- [x] 3.1 Import only sanitized public-safe evidence into `reports/public-sample/a100-route-ontology-train-split-rerun/`.
- [x] 3.2 Generate metrics, schema guard summary, route ontology diagnosis, manifest, report Markdown, and leak-scan results that separate raw attempt, retry attempt, validated output source, route validity, prompt constraints, and final metrics.
