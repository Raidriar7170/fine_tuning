...
def test_a100_first_pass_fence_suppression_rerun_pack_is_public_safe_and_bounded() -> None:
    prior_dir = Path("reports/public-sample/a100-first-pass-output-boundary-rerun")
    local_suppression_dir = Path("reports/public-sample/first-pass-markdown-fence-suppression")
    evidence_dir = Path("reports/public-sample/a100-first-pass-fence-suppression-rerun")
    human_brief_path = Path("docs/human-briefs/2026-06-08-run-a100-first-pass-fence-suppression-rerun.html")
    archive_dir = Path("openspec/changes/archive/2026-06-08-run-a100-first-pass-fence-suppression-rerun")
    change_dirs = [Path("openspec/changes/run-a100-first-pass-fence-suppression-rerun"), archive_dir]
    required_files = {
        "predictions.jsonl",
        "prediction_metadata.json",
        "prompt_snapshot.json",
        ...
        "alignment_diagnostics.md",
        "schema_guard_summary.json",
        "schema_guard_summary.md",
        "fence_suppression_rerun_diagnosis.json",
        "fence_suppression_rerun_diagnosis.md",
        "manifest.json",
        "report.md",
        "leak_scan_result.json",
        "phase_validation_leak_scan_result.json",
    ...
    metrics = json.loads((evidence_dir / "metrics.json").read_text(encoding="utf-8"))
    schema_guard = json.loads((evidence_dir / "schema_guard_summary.json").read_text(encoding="utf-8"))
    diagnosis = json.loads((evidence_dir / "fence_suppression_rerun_diagnosis.json").read_text(encoding="utf-8"))
    manifest = json.loads((evidence_dir / "manifest.json").read_text(encoding="utf-8"))
    alignment = json.loads((evidence_dir / "alignment_diagnostics.json").read_text(encoding="utf-8"))
    prediction_rows = [
        json.loads(line)
        for line in (evidence_dir / "predictions.jsonl").read_text(encoding="utf-8").splitlines()
        if line.strip()
    ...
    report = (evidence_dir / "report.md").read_text(encoding="utf-8")
    diagnosis_markdown = (evidence_dir / "fence_suppression_rerun_diagnosis.md").read_text(encoding="utf-8")
    human_brief = human_brief_path.read_text(encoding="utf-8")
    ...
    assert metadata["decoding_policy"]["markdown_fence_suppression_token_sources"] == ["```", "```json", "```JSON"]
    assert prompt_snapshot["decoding_policy"] == metadata["decoding_policy"]
    assert [row["id"] for row in prompt_snapshot["rows"]] == expected_row_ids
    assert [row["id"] for row in prediction_rows] == expected_row_ids
    assert [row["id"] for row in train_gold_rows] == expected_row_ids

    assert metrics["metrics"]["json_valid_rate"] == 1.0
    assert metrics["metrics"]["contract_exact_match"] == 2 / 3
    assert metrics["metrics"]["slot_f1"] == 2 / 3
    assert metrics["failure_slices"]["schema"]["count"] == 0
    assert metrics["failure_slices"]["slot"]["count"] == 1
    assert alignment["summary"]["schema_invalid_prediction_count"] == 0
    assert alignment["summary"]["row_mismatch_count"] == 1
    assert alignment["summary"]["field_mismatch_counts"] == {"slots": 1}

    assert diagnosis["diagnostic_kind"] == "a100_first_pass_fence_suppression_rerun"
    assert diagnosis["source_artifacts"]["prior_a100_first_pass_output_boundary_rerun"] == prior_dir.as_posix()
    assert diagnosis["source_artifacts"]["local_markdown_fence_suppression"].startswith(
        local_suppression_dir.as_posix()
    )
    assert diagnosis["summary"]["prediction_count"] == 3
    assert diagnosis["summary"]["markdown_fence_suppression_enabled"] is True
    assert diagnosis["summary"]["markdown_fence_suppression_strategy"] == "bad_words_ids"
    assert diagnosis["summary"]["markdown_wrapped_prediction_count"] == 0
    assert diagnosis["summary"]["validated_output_schema_valid_count"] == 3
    assert diagnosis["summary"]["raw_attempt_schema_valid_count"] == 3
    assert diagnosis["summary"]["retry_attempted_count"] == 0
    assert diagnosis["summary"]["validated_output_source_counts"] == {"raw_attempt": 3}
    assert diagnosis["summary"]["strict_final_json_valid_rate"] == metrics["metrics"]["json_valid_rate"]
    assert diagnosis["summary"]["strict_final_contract_exact_match"] == metrics["metrics"]["contract_exact_match"]
    assert diagnosis["summary"]["strict_final_slot_f1"] == metrics["metrics"]["slot_f1"]
    assert diagnosis["summary"]["slot_exact_match_residual_rows"] == 1
    assert diagnosis["comparison_to_prior"]["prior_markdown_wrapped_prediction_count"] == 3
    assert diagnosis["comparison_to_prior"]["current_markdown_wrapped_prediction_count"] == 0
    assert diagnosis["comparison_to_prior"]["prior_validated_output_schema_valid_count"] == 0
    assert diagnosis["comparison_to_prior"]["current_validated_output_schema_valid_count"] == 3
    assert diagnosis["comparison_to_prior"]["wrapper_reduction_observed"] is True
    assert diagnosis["comparison_to_prior"]["schema_recovery_vs_prior_observed"] is True
    assert diagnosis["interpretation"]["not_a_model_quality_improvement_claim"] is True
    assert diagnosis["interpretation"]["not_a_held_out_generalization_claim"] is True
    assert diagnosis["claims"]["a100_execution_performed"] is True
    assert diagnosis["claims"]["prediction_rerun_performed"] is True
    assert diagnosis["claims"]["training_performed"] is False
    assert diagnosis["claims"]["parser_relaxation_performed"] is False
    ...
    assert schema_guard["evidence_kind"] == "a100_first_pass_fence_suppression_schema_guard_summary"
    assert schema_guard["summary"] == diagnosis["summary"]
    assert schema_guard["parse_status_counts"]["raw_attempt"] == {"json_object": 3}
    assert schema_guard["parse_status_counts"]["retry_attempt"] == {}
    assert [row["row_id"] for row in schema_guard["rows"]] == expected_row_ids
    assert all(row["strict_schema_valid"] is True for row in schema_guard["rows"])
    assert all(row["strict_prediction_is_markdown_wrapped"] is False for row in schema_guard["rows"])
    assert all(row["validated_output_source"] == "raw_attempt" for row in schema_guard["rows"])

    assert [row["parse_status"] for row in raw_rows] == ["json_object", "json_object", "json_object"]
    assert [row["schema_guard"]["retry_attempted"] for row in raw_rows] == [False, False, False]
    assert [row["attempt"] for row in generation_trace_rows] == ["raw_attempt", "raw_attempt", "raw_attempt"]
    assert all(row["finish_state"] == "eos_observed" for row in generation_trace_rows)
    assert all(row["max_new_tokens_hit"] is False for row in generation_trace_rows)

    assert manifest["evidence_kind"] == "a100_first_pass_fence_suppression_train_split_rerun"
    assert manifest["observed_result"] == diagnosis["summary"]
    assert manifest["claims"] == diagnosis["claims"]
    assert manifest["source_artifacts"] == diagnosis["source_artifacts"]
    assert manifest["artifact_policy"]["private_configs_copied_to_git"] is False
    assert manifest["artifact_policy"]["raw_logs_copied_to_git"] is False
    for artifact_path in manifest["artifacts"].values():
        assert Path(artifact_path).exists()
    ...
    assert phase_validation_leak_scan["ok"] is True
    assert phase_validation_leak_scan["findings"] == []
    assert "0/3 Markdown-wrapped predictions" in report
    assert "not a held-out generalization claim" in diagnosis_markdown
...
def test_fence_suppression_slot_residual_diagnosis_pack_is_public_safe_and_bounded() -> None:
    source_dir = Path("reports/public-sample/a100-first-pass-fence-suppression-rerun")
    evidence_dir = Path("reports/public-sample/fence-suppression-slot-residual-diagnosis")
    human_brief_path = Path("docs/human-briefs/2026-06-08-diagnose-fence-suppression-slot-residual.html")
    archive_dir = Path("openspec/changes/archive/2026-06-08-diagnose-fence-suppression-slot-residual")
    change_dirs = [
        Path("openspec/changes/diagnose-fence-suppression-slot-residual"),
        archive_dir,
    ]
    required_files = {
        "slot_residual_diagnosis.json",
        "slot_residual_diagnosis.md",
        "manifest.json",
        "leak_scan_result.json",
        "phase_validation_leak_scan_result.json",
    }
    ...
    diagnosis = json.loads((evidence_dir / "slot_residual_diagnosis.json").read_text(encoding="utf-8"))
    manifest = json.loads((evidence_dir / "manifest.json").read_text(encoding="utf-8"))
    report = (evidence_dir / "slot_residual_diagnosis.md").read_text(encoding="utf-8")
    human_brief = human_brief_path.read_text(encoding="utf-8")
    leak_scan = json.loads((evidence_dir / "leak_scan_result.json").read_text(encoding="utf-8"))
    phase_validation_leak_scan = json.loads(
        (evidence_dir / "phase_validation_leak_scan_result.json").read_text(encoding="utf-8")
    ...
    source_alignment = json.loads((source_dir / "alignment_diagnostics.json").read_text(encoding="utf-8"))
    source_metrics = json.loads((source_dir / "metrics.json").read_text(encoding="utf-8"))
    source_schema_guard = json.loads((source_dir / "schema_guard_summary.json").read_text(encoding="utf-8"))

    assert diagnosis["evidence_kind"] == "fence_suppression_slot_residual_diagnosis_local"
    assert diagnosis["diagnostic_kind"] == "fence_suppression_slot_exact_match_residual"
    assert diagnosis["source_prior_phase"] == source_dir.as_posix()
    assert diagnosis["source_artifacts"]["alignment_diagnostics"].endswith("alignment_diagnostics.json")
    assert diagnosis["source_artifacts"]["schema_guard_summary"].endswith("schema_guard_summary.json")
    assert diagnosis["source_artifact_policy"]["uses_prior_public_sample_artifacts_only"] is True
    assert diagnosis["source_artifact_policy"]["no_private_raw_logs_or_adapters_read"] is True

    assert diagnosis["summary"]["residual_row_count"] == source_alignment["summary"]["row_mismatch_count"] == 1
    assert diagnosis["summary"]["residual_row_ids"] == ["seed-search-weather-aug-1"]
    assert diagnosis["summary"]["field_mismatch_counts"] == source_alignment["summary"]["field_mismatch_counts"] == {
        "slots": 1
    }
    assert diagnosis["summary"]["schema_invalid_prediction_count"] == 0
    assert diagnosis["summary"]["validated_output_schema_valid_count"] == 3
    assert diagnosis["summary"]["strict_metric_boundary"]["json_valid_rate"] == source_metrics["metrics"][
        "json_valid_rate"
    ] == 1.0
    assert diagnosis["summary"]["strict_metric_boundary"]["contract_exact_match"] == source_metrics["metrics"][
        "contract_exact_match"
    ] == 2 / 3
    assert diagnosis["summary"]["strict_metric_boundary"]["slot_f1"] == source_metrics["metrics"]["slot_f1"] == 2 / 3
    assert diagnosis["source_summary"]["markdown_wrapped_prediction_count"] == 0
    assert diagnosis["source_summary"]["validated_output_schema_valid_count"] == source_schema_guard["summary"][
        "validated_output_schema_valid_count"
    ] == 3

    assert [row["row_id"] for row in diagnosis["rows"]] == ["seed-search-weather-aug-1"]
    residual = diagnosis["rows"][0]
    assert residual["mismatch_category"] == "strict_slot_object_mismatch"
    assert residual["gold_slots"] == {"query": "北京明天天气"}
    assert residual["prediction_slots"] == {"city": "北京", "date": "明天", "topic": ""}
    assert residual["gold_slot_keys"] == ["query"]
    assert residual["prediction_slot_keys"] == ["city", "date", "topic"]
    assert residual["strict_contract_exact_match"] is False
    assert residual["schema_guard"]["strict_schema_valid"] is True
    assert residual["schema_guard"]["strict_prediction_is_markdown_wrapped"] is False
    assert residual["schema_guard"]["retry_attempted"] is False
    assert residual["metric_impact"]["contributes_contract_exact_match_failure"] is True
    assert residual["metric_impact"]["contributes_slot_failure_slice"] is True

    assert diagnosis["interpretation"]["semantic_relation_is_not_counted_as_pass"] is True
    assert diagnosis["next_recommended_decision"]["needs_user_confirmation_before_behavior_change"] is True
    ...
    assert manifest["evidence_kind"] == "fence_suppression_slot_residual_diagnosis_local"
    assert manifest["source_prior_phase"] == source_dir.as_posix()
    assert manifest["counts"]["residual_row_count"] == 1
    assert manifest["diagnostic_artifacts"]["diagnosis"].endswith("slot_residual_diagnosis.json")
    assert manifest["diagnostic_artifacts"]["report"].endswith("slot_residual_diagnosis.md")
    assert manifest["diagnostic_artifacts"]["phase_validation_leak_scan"].endswith(
        "phase_validation_leak_scan_result.json"
    )
    ...
    assert "semantic relation" in report
    assert "No A100 execution was performed" in report
    assert "本地 evidence-only diagnosis" in human_brief
    assert "不改 strict evaluator metrics" in human_brief
    assert "语义相关，但在 strict evaluator 下仍不是 exact match" in human_brief
...
def test_a100_compact_query_slot_preservation_rerun_pack_is_public_safe_and_bounded() -> None:
    prior_dir = Path("reports/public-sample/a100-first-pass-fence-suppression-rerun")
    local_preservation_dir = Path("reports/public-sample/compact-query-slot-preservation")
    evidence_dir = Path("reports/public-sample/a100-compact-query-slot-preservation-rerun")
    human_brief_path = Path(
        "docs/human-briefs/2026-06-08-run-a100-compact-query-slot-preservation-rerun.html"
    )
    archive_dir = Path("openspec/changes/archive/2026-06-08-run-a100-compact-query-slot-preservation-rerun")
    change_dirs = [
        Path("openspec/changes/run-a100-compact-query-slot-preservation-rerun"),
        archive_dir,
    ]
    required_files = {
        "predictions.jsonl",
        ...
        "schema_guard_summary.json",
        "schema_guard_summary.md",
        "compact_query_slot_preservation_rerun_diagnosis.json",
        "compact_query_slot_preservation_rerun_diagnosis.md",
        "manifest.json",
        "report.md",
        "leak_scan_result.json",
        "phase_validation_leak_scan_result.json",
    ...
    schema_guard = json.loads((evidence_dir / "schema_guard_summary.json").read_text(encoding="utf-8"))
    diagnosis = json.loads(
        (evidence_dir / "compact_query_slot_preservation_rerun_diagnosis.json").read_text(encoding="utf-8")
    )
    manifest = json.loads((evidence_dir / "manifest.json").read_text(encoding="utf-8"))
    prediction_rows = [
        json.loads(line)
    ...
    report = (evidence_dir / "report.md").read_text(encoding="utf-8")
    metrics_markdown = (evidence_dir / "metrics.md").read_text(encoding="utf-8")
    schema_guard_markdown = (evidence_dir / "schema_guard_summary.md").read_text(encoding="utf-8")
    diagnosis_markdown = (evidence_dir / "compact_query_slot_preservation_rerun_diagnosis.md").read_text(
        encoding="utf-8"
    )
    human_brief = human_brief_path.read_text(encoding="utf-8")
    leak_scan = json.loads((evidence_dir / "leak_scan_result.json").read_text(encoding="utf-8"))
    ...
    source_diagnosis = json.loads((prior_dir / "fence_suppression_rerun_diagnosis.json").read_text(encoding="utf-8"))
    local_preservation = json.loads(
        (local_preservation_dir / "compact_query_slot_preservation.json").read_text(encoding="utf-8")
    )
    serialized = "\n".join(
        [
            json.dumps(metadata, ensure_ascii=False, sort_keys=True),
    ...
    assert metadata["adapter_release_status"] == "not_released"
    assert metadata["prompt_constraints"]["compact_search_query_slot_policy_visible"] is True
    assert metadata["prompt_constraints"]["search_query_no_city_date_split_visible"] is True
    assert metadata["prompt_constraints"]["decomposed_search_slots_rejected_visible"] is True
    assert metadata["prompt_constraints"]["policy_is_target_formatting_not_evaluator_normalization"] is True
    assert prompt_snapshot["prompt_constraints"]["compact_search_query_slot_policy_visible"] is True
    assert prompt_snapshot["prompt_constraints"]["search_query_no_city_date_split_visible"] is True
    assert prompt_snapshot["prompt_constraints"]["decomposed_search_slots_rejected_visible"] is True
    assert prompt_snapshot["prompt_constraints"]["policy_is_target_formatting_not_evaluator_normalization"] is True

    assert [row["id"] for row in prediction_rows] == expected_row_ids
    assert [row["id"] for row in raw_rows] == expected_row_ids
    assert [row["id"] for row in train_gold_rows] == expected_row_ids
    assert [row["id"] for row in prompt_snapshot["rows"]] == expected_row_ids
    assert {row["id"] for row in generation_trace_rows} == set(expected_row_ids)
    assert {row["attempt"] for row in generation_trace_rows} <= {"raw_attempt", "retry_attempt"}

    train_gold_by_id = {row["id"]: row for row in train_gold_rows}
    key_gold_contract = train_gold_by_id["seed-search-weather-aug-1"]["target_contract"]
    assert key_gold_contract["slots"] == {"query": "北京明天天气"}
    assert key_gold_contract["normalized_command"] == "搜索北京明天天气"
    assert not ({"city", "date", "topic"} & set(key_gold_contract["slots"]))
    for row in train_gold_rows:
        assert row["target_contract"]["slots"] == {"query": "北京明天天气"}

    assert metrics["metrics"]["json_valid_rate"] == diagnosis["summary"]["strict_final_json_valid_rate"]
    assert metrics["metrics"]["contract_exact_match"] == diagnosis["summary"]["strict_final_contract_exact_match"]
    assert metrics["metrics"]["slot_f1"] == diagnosis["summary"]["strict_final_slot_f1"]
    assert schema_guard["summary"]["prediction_count"] == 3
    assert schema_guard["summary"]["prediction_split"] == "train"
    assert schema_guard["summary"]["prediction_source_kind"] == "private_a100_adapter"
    assert schema_guard["summary"]["strict_final_json_valid_rate"] == metrics["metrics"]["json_valid_rate"]
    assert schema_guard["summary"]["strict_final_contract_exact_match"] == metrics["metrics"]["contract_exact_match"]

    assert diagnosis["diagnostic_kind"] == "a100_compact_query_slot_preservation_rerun"
    assert diagnosis["evidence_kind"] == "a100_compact_query_slot_preservation_train_split_rerun"
    assert diagnosis["source_prior_phase"] == prior_dir.as_posix()
    assert diagnosis["source_artifacts"]["prior_a100_first_pass_fence_suppression_rerun"] == prior_dir.as_posix()
    assert diagnosis["source_artifacts"]["local_compact_query_slot_preservation"].startswith(
        local_preservation_dir.as_posix()
    )
    assert diagnosis["source_artifacts"]["public_sample_manifest"] == "data/public-samples/manifest_public_sample.json"
    assert diagnosis["source_residual"]["row_id"] == "seed-search-weather-aug-1"
    assert diagnosis["source_residual"]["gold_slots"] == {"query": "北京明天天气"}
    assert diagnosis["source_residual"]["prediction_slots"] == {"city": "北京", "date": "明天", "topic": ""}
    assert diagnosis["source_residual"]["strict_contract_exact_match"] is False
    assert diagnosis["source_residual"] == local_preservation["source_residual"]
    assert diagnosis["source_residual_policy"]["historical_predictions_preserved"] is True
    assert diagnosis["source_residual_policy"]["historical_metrics_preserved"] is True
    assert diagnosis["source_residual_policy"]["not_reinterpreted_as_exact_match_recovery"] is True
    assert diagnosis["comparison_to_prior"]["prior_strict_final_contract_exact_match"] == source_diagnosis["summary"][
        "strict_final_contract_exact_match"
    ]
    assert diagnosis["comparison_to_prior"]["prior_seed_search_weather_aug_1_prediction_slots"] == {
        "city": "北京",
        "date": "明天",
        "topic": "",
    }

    target_summary = diagnosis["summary"]["target_residual_row"]
    assert target_summary["row_id"] == "seed-search-weather-aug-1"
    assert target_summary["gold_slots"] == {"query": "北京明天天气"}
    assert target_summary["prior_prediction_slots"] == {"city": "北京", "date": "明天", "topic": ""}
    assert target_summary["strict_contract_exact_match"] is False
    assert target_summary["outcome"] == "strict_mismatch_preserved"
    assert target_summary["prediction_slots"] == {"city": "北京", "date": "明天", "topic": ""}
    assert target_summary["mismatch_preserved_without_repair"] is True

    diagnosis_rows_by_id = {row["row_id"]: row for row in diagnosis["rows"]}
    key_diagnosis_row = diagnosis_rows_by_id["seed-search-weather-aug-1"]
    assert key_diagnosis_row["gold_slots"] == {"query": "北京明天天气"}
    assert key_diagnosis_row["source_prior_prediction_slots"] == {"city": "北京", "date": "明天", "topic": ""}
    assert key_diagnosis_row["strict_contract_exact_match"] == target_summary["strict_contract_exact_match"]
    assert diagnosis["summary"]["prediction_count"] == 3
    assert diagnosis["summary"]["prediction_split"] == "train"
    assert diagnosis["summary"]["prediction_source_kind"] == "private_a100_adapter"
    assert diagnosis["summary"]["gold_compact_query_count"] == 3
    assert diagnosis["summary"]["gold_city_date_slot_count"] == 0
    assert diagnosis["summary"]["exact_match_count"] == int(metrics["metrics"]["contract_exact_match"] * 3)
    assert diagnosis["interpretation"]["train_split_only_diagnostic"] is True
    assert diagnosis["interpretation"]["not_a_held_out_generalization_claim"] is True
    assert diagnosis["interpretation"]["not_a_model_quality_improvement_claim"] is True

    claims = diagnosis["claims"]
    assert claims["a100_execution_performed"] is True
    assert claims["prediction_rerun_performed"] is True
    assert claims["training_performed"] is False
    assert claims["train_split_only"] is True
    assert claims["parser_relaxation_performed"] is False
    assert claims["evaluator_metric_change_performed"] is False
    assert claims["schema_repair_or_coercion_applied"] is False
    assert claims["slot_normalization_performed"] is False
    assert claims["semantic_equivalence_scoring_performed"] is False
    assert claims["prediction_repair_or_rescore_performed"] is False
    assert claims["held_out_generalization_claim"] is False
    assert claims["model_quality_improvement_claim"] is False
    assert claims["model_recovery_claim"] is False
    assert claims["checkpoint_release"] is False
    assert claims["adapter_release"] is False
    assert claims["public_full_corpus_release_claim"] is False
    assert claims["production_readiness_claim"] is False
    assert claims["live_browser_benchmark_claim"] is False

    assert manifest["evidence_kind"] == "a100_compact_query_slot_preservation_train_split_rerun"
    assert manifest["observed_result"] == diagnosis["summary"]
    assert manifest["source_artifacts"] == diagnosis["source_artifacts"]
    assert manifest["claims"] == claims
    assert manifest["validation_commands"] == diagnosis["validation_commands"]
    assert any(
        command["command"] == "PYTHONPATH=src pytest -q" and command["result"] == "193 passed"
        for command in manifest["validation_commands"]
    )
    assert any(
        command["command"] == "OPENSPEC_TELEMETRY=0 openspec validate --all --strict"
        and command["result"] == "5 passed, 0 failed"
        for command in manifest["validation_commands"]
    )
    assert manifest["artifact_policy"]["a100_execution_performed"] is True
    assert manifest["artifact_policy"]["prediction_rerun_performed"] is True
    ...
    assert "A100 prediction-only train-split diagnostic" in report
    assert "strict compact slots.query" in report
    assert "train-split-only prediction diagnostic" in diagnosis_markdown
    assert "not a held-out generalization claim" in diagnosis_markdown
    assert "not model-quality evidence" in diagnosis_markdown
    assert "A100 prediction-only" in human_brief
    assert "不做 slot normalization" in human_brief
    assert "不能声明 held-out generalization" in human_brief
    assert "不发布 checkpoint" in human_brief
...
def test_first_pass_wrapper_persistence_diagnosis_pack_is_public_safe_and_bounded() -> None:
    source_dir = Path("reports/public-sample/a100-first-pass-output-boundary-rerun")
    evidence_dir = Path("reports/public-sample/a100-first-pass-wrapper-persistence-diagnosis")
    human_brief_path = Path("docs/human-briefs/2026-06-08-diagnose-first-pass-wrapper-persistence.html")
    archive_dir = Path("openspec/changes/archive/2026-06-08-diagnose-first-pass-wrapper-persistence")
    change_dirs = [
        Path("openspec/changes/diagnose-first-pass-wrapper-persistence"),
        archive_dir,
    ]
    required_files = {
        "wrapper_persistence_diagnosis.json",
        "wrapper_persistence_diagnosis.md",
        "manifest.json",
        "leak_scan_result.json",
        "phase_validation_leak_scan_result.json",
    }
    ...
    diagnosis = json.loads((evidence_dir / "wrapper_persistence_diagnosis.json").read_text(encoding="utf-8"))
    manifest = json.loads((evidence_dir / "manifest.json").read_text(encoding="utf-8"))
    report = (evidence_dir / "wrapper_persistence_diagnosis.md").read_text(encoding="utf-8")
    human_brief = human_brief_path.read_text(encoding="utf-8")
    leak_scan = json.loads((evidence_dir / "leak_scan_result.json").read_text(encoding="utf-8"))
    phase_validation_leak_scan = json.loads(
        (evidence_dir / "phase_validation_leak_scan_result.json").read_text(encoding="utf-8")
    ...
    source_diagnosis = json.loads((source_dir / "output_boundary_rerun_diagnosis.json").read_text(encoding="utf-8"))
    source_schema_guard = json.loads((source_dir / "schema_guard_summary.json").read_text(encoding="utf-8"))
    serialized = "\n".join(
        [
            json.dumps(diagnosis, ensure_ascii=False, sort_keys=True),
    ...
    assert diagnosis["evidence_kind"] == "a100_first_pass_wrapper_persistence_diagnosis_local"
    assert diagnosis["diagnostic_kind"] == "first_pass_wrapper_persistence_diagnosis"
    assert diagnosis["source_prior_phase"] == source_dir.as_posix()
    assert diagnosis["source_artifacts"]["source_diagnosis"].endswith("output_boundary_rerun_diagnosis.json")
    assert diagnosis["source_artifacts"]["source_schema_guard_summary"].endswith("schema_guard_summary.json")
    assert diagnosis["source_artifact_policy"]["uses_source_public_sample_artifacts_only"] is True
    assert diagnosis["source_artifact_policy"]["source_predictions_preserved"] is True
    assert diagnosis["source_artifact_policy"]["source_metrics_preserved"] is True
    assert diagnosis["source_artifact_policy"]["no_private_raw_logs_or_adapters_read"] is True
    ...
    assert summary["prediction_count"] == 3
    assert summary["row_ids"] == expected_row_ids
    assert summary["prediction_output_boundary_visible"] is True
    assert summary["markdown_wrapped_prediction_count"] == 3
    assert summary["validated_output_schema_valid_count"] == 0
    assert summary["strict_final_json_valid_rate"] == 0
    assert summary["raw_attempt_schema_valid_count"] == 0
    assert summary["retry_attempt_schema_valid_count"] == 0
    assert summary["raw_parse_status_counts"] == {"json_fragment_object": 3}
    assert summary["retry_parse_status_counts"] == {"json_fragment_object": 3}
    ...
    assert summary["raw_generation_max_new_tokens_hit_count"] == 0
    assert summary["retry_generation_max_new_tokens_hit_count"] == 0
    assert summary["wrapper_persistence_observed"] is True
    assert summary["strict_parser_accepts_markdown_wrappers"] is False
    assert summary["output_boundary_visibility_not_sufficient"] is True
    assert summary["source_schema_guard_summary"] == source_schema_guard["summary"]
    assert summary["source_observed_result"] == source_diagnosis["summary"]

    assert [row["row_id"] for row in diagnosis["rows"]] == expected_row_ids
    for row in diagnosis["rows"]:
        assert row["boundary_context"]["prediction_output_boundary_visible"] is True
        assert row["raw_attempt"]["parse_status"] == "json_fragment_object"
        assert row["retry_attempt"]["parse_status"] == "json_fragment_object"
        assert row["raw_generation"]["finish_state"] == "eos_observed"
        assert row["retry_generation"]["finish_state"] == "eos_observed"
    ...
    assert diagnosis["interpretation"]["eos_observed_does_not_mean_schema_valid"] is True
    assert diagnosis["interpretation"]["boundary_prompt_visibility_did_not_remove_wrappers"] is True
    assert diagnosis["evidence_gaps"]["wrapper_origin_unproven"] is True
    assert diagnosis["evidence_gaps"]["causal_fix_unproven"] is True
    assert diagnosis["claims"]["local_diagnosis_only"] is True
    assert diagnosis["claims"]["a100_execution_performed"] is False
    assert diagnosis["claims"]["training_or_prediction_rerun_performed"] is False
    assert diagnosis["claims"]["parser_relaxation_performed"] is False
    ...
    assert manifest["counts"]["markdown_wrapped_predictions"] == 3
    assert manifest["counts"]["validated_output_schema_valid"] == 0
    assert manifest["diagnostic_artifacts"]["diagnosis"].endswith("wrapper_persistence_diagnosis.json")
    assert manifest["diagnostic_artifacts"]["report"].endswith("wrapper_persistence_diagnosis.md")
    assert manifest["claims"] == diagnosis["claims"]
    ...
    if archive_dir.exists():
        ...
        assert final_leak_scan["findings"] == []
    assert "local evidence-only diagnosis" in report
    assert "EOS observed does not mean schema-valid output" in report
    assert "不启动新的 A100" in human_brief
    assert "不能声明模型质量改善" in human_brief
    assert "/mnt/data/" not in serialized
    assert "/Users/" not in serialized
