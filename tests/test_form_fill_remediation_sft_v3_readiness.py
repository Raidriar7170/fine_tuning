...
def test_form_fill_sft_v3_dry_run_selects_current_train_split_and_merged_form_fill_rows(
    ...
    assert metadata["training_rows_used"] == current_manifest["split_counts"]["train"] == 114
    assert metadata["training_rows_before_source_filter"] == 114
...
def test_form_fill_sft_v3_readiness_report_cli_writes_public_safe_non_claiming_evidence(
    ...
    assert evidence["summary"]["dataset_manifest_id"] == "public-sample-20260616T074315Z"
    assert evidence["summary"]["training_rows_used"] == 114
...
def test_committed_form_fill_sft_v3_readiness_evidence_is_current_and_non_claiming() -> None:
    ...
    assert dry_run["training_rows_used"] == 114
