...
| BLOCKED | must-fix-phase-3-lockbox-lineage-guard | deterministic lockbox validator and one-look rule documentation; no real frozen lockbox rows committed | Workflow implemented for row hashes, external row ID overlap, lineage overlap checks, strict manifest frozen/schema/timestamp/provenance/family/hash/count checks, and JSON CLI validation; final lockbox evaluation blocked until real new content is authored and frozen; no current model metrics. | - | `docs/lockbox.md` |
