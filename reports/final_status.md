...
如果项目继续推进，下一步应开独立的 `run-a100-form-fill-remediation-sft-v3` OpenSpec phase：先做 A100 GPU preflight 和 private override，再训练一个私有 SFT v3 adapter，随后用 strict held-out dev/test 做 prediction-only 评估。不要重跑 DPO、修改 evaluator，或把 `slot_f1_soft` 当作公开主指标。
...
可以作为阶段性交付收束：代码、public sample、OpenSpec archives、A100 prediction-only evidence、residual diagnosis / target selection、SFT v3 readiness、Human Brief、最终状态说明都已经对齐到同一套边界。下一步如果继续，应由单独的 `run-a100-form-fill-remediation-sft-v3` phase 执行，而不是从当前报告直接启动无边界训练。
