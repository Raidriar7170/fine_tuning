...
| Latest safety repair recommended next step | `review_safety_repair_candidates_before_materialization` |
...
当前最新推荐来自 `design-safety-repair-candidates`：

- 输入：`reports/public-sample/safety-repair-candidate-design/safety_repair_candidate_design.json`、
  `reports/public-sample/safety-repair-candidate-design/safety_repair_candidate_design.md` 和
  `reports/public-sample/safety-repair-candidate-design/manifest.json`；
- 推荐下一步：`review_safety_repair_candidates_before_materialization`；
- 目的：人工审阅 3 个 safety repair candidate themes，确认哪些主题可以进入后续独立 materialization 或 safety-policy phase；
