...
当前最新推荐来自 `review-safety-repair-candidates-before-materialization`：

- 输入：`reports/public-sample/safety-repair-candidate-design-review/safety_repair_candidate_design_review.json`、
  `reports/public-sample/safety-repair-candidate-design-review/safety_repair_candidate_design_review.md` 和
  `reports/public-sample/safety-repair-candidate-design-review/manifest.json`；
- 推荐下一步：`propose_clarify_confirmation_safety_repair_materialization_after_review`；
- 目的：只围绕 row-backed `clarify_confirmation_preservation` 开后续独立 materialization proposal；broader `unsafe_action_denial_boundary` 先走 safety-policy design；
...
- 后续：若要进入 seed materialization、formal sample merge、训练或 policy/canonicalization 变更，应该先开
