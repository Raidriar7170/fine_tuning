...
| Latest canonical slot-boundary formal-merge proposal result | proposal/readiness only; `formal_merge_ready_now=false`, `formal_merge_readiness=not_ready_missing_row_level_candidate_source`, `implemented_now=false` |
| Latest canonical slot-boundary formal-merge proposal recommended next step | materialize exact row-level source `data/public-samples/canonical_slot_boundary_seed_candidates.jsonl`; do not direct-merge |
...
- required future source artifact: `data/public-samples/canonical_slot_boundary_seed_candidates.jsonl`，但本阶段不创建；
...
- recommended next bounded step: 先开很小的 materialization phase，生成 exact reviewed row-level canonical slot-boundary seed candidates；不要直接 merge。
...
当前最新推荐来自 `propose-canonical-slot-boundary-formal-merge-after-review`：

- 输入：`reports/public-sample/canonical-slot-boundary-formal-merge-proposal/summary.json`、
  `reports/public-sample/canonical-slot-boundary-formal-merge-proposal/summary.md` 和
  `reports/public-sample/canonical-slot-boundary-candidate-review/summary.json`；
- 推荐下一步：单独开 materialization phase，生成 exact reviewed row-level source
  `data/public-samples/canonical_slot_boundary_seed_candidates.jsonl`；
- 目的：只补齐 formal merge 的行级候选源；不要直接合并 formal public sample；
