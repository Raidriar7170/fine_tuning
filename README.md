...
| Copy-shadow status | Challenge v1 is an adversarial verifier fixture, not a naturalistic benchmark. Policy V2 freeze is `POLICY_V2_FROZEN_INACTIVE_REFERENCE_READY_FOR_NATURALISTIC_CHALLENGE_DESIGN`; frozen statuses are form fill `PROPOSE_DISABLE`, search `INSUFFICIENT_EVIDENCE`, and extract `INSUFFICIENT_EVIDENCE`. It is inactive, not runtime loaded, and execution-eligible count remains 0. |
